...
func _ready() -> void:
	var reopen : Callable = Callable(get_node("/root/Main"), &"open_app_page").bind(get_node("/root/Main").open_app)
	var prod_key : String = APIManager.product.find_key(get_node("/root/Main").open_app)
